...
# load modules already in initrd on non-Optimus, i.e. nVidia-only systems
if ! lspci -n | grep -e '^..:..\.. 0300: ' | cut -d " "  -f3 | cut -d ":" -f1 | grep -q -E "8086|1002"; then
  sed -i 's/omit_drivers/add_drivers/' $dracut_file
fi
